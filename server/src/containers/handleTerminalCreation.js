export const handleTerminalCreation = async (container, ws) => {
  ...
  exec.start({ hijack: true, stdin: true }, (err, stream) => {
    // For TTY mode, stream data is not multiplexed
    stream.on("data", (data) => {
...
function processStreamOutput(stream, ws) {
  let nextDataType = null; // it stores the type of next data message
  let nextDataLength = null; // it stores the length of next data message
  ...
  function processStreamData(data) {
    ...
    if (!nextDataType) {
      ...
      if (buffer.length >= 8) {
        ...
        nextDataType = header.readUInt8(0); // the first byte represents the stream type (0=stdin, 1=stdout, 2=stderr)
        nextDataLength = header.readUInt32BE(4); // bytes 4-7 represent the length of next data message
